...
## Interactive map and chart G, 2026-09-03

**Interactive map.** CARTO began requiring an API key for raster basemaps and serves a watermarked
"API KEY REQUIRED" tile instead of an error — HTTP 200, byte-identical with a wrong key or none, so
a status-code check does not detect it. Voyager and Positron were removed rather than carrying a key
that would be public in this repo (readable in page source, permanent in git history) for a product
CARTO is retiring. OpenTopoMap was already present, is keyless, and is now the only and default
basemap; the terrain shading also carries the water-constraint argument. Verified: tile requests go
only to opentopomap.org.

**Legend clipping.** Not a legend problem. `#htmlwidget_container` is `position:absolute;inset:0`
holding a title header plus a `height:100%` map, so the map overflowed its container by exactly the
header height (110px desktop; the header grows as it wraps on narrow screens) and the bottom-right
legend fell outside. Fixed with a flex column so the map takes the remaining height at any header
size. Verified desktop (overflow 110 -> 0) and mobile (header 92px, legend visible).

**Chart G was upside down.** `scripts/viz_style.R` called
`rasterImage(raster, xmin, ymax, xmax, ymin)` where the signature is
`(image, xleft, ybottom, xright, ytop)` — ymax passed as ybottom and ymin as ytop, drawing every
basemap vertically mirrored behind correctly drawn polygons. Place labels read backwards, Oklahoma
City sat at the bottom, Monterrey at the top. Present since the June design-system pass and
published on section 04. Fixed in `viz_style.R`; `BASEMAP_PATH` now resolves from the repo instead
of `~/rco_run` on one machine. Chart regenerated by `profile_2026_09/regen_chart_G_2026-09.R`, which
reads everything from the repo and takes county geometry from the Census cb_2023_us_county_20m
shapefile rather than requiring `tigris`.

`slides/img/F_tx_siting_risk.png` had the same flip but is no longer referenced — section 09 was
retired. **The June archive keeps its own flipped copies and must not be corrected:** it is the
